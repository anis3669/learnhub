...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($lessonsByCourse as $courseId => $lessonList) {
        ...
        $allCourses = [$course1, $course2, $course3, $course4];
        foreach ($students as $i => $student) {
            $numCourses = min(3, $i + 1);
            foreach (array_slice($allCourses, 0, $numCourses) as $course) {
                Enrollment::firstOrCreate(
                    ['user_id' => $student->id, 'course_id' => $course->id],
                    ['progress_percent' => rand(10, 90), 'prerequisites_met' => true]
                );
            }
        }
        ...
        // Run new intelligent LMS seeders
        $this->call([
